...
class UIHome():
    ...
    def __init__(self, window, b1Action, b2Action, b3Action):
        ...
        tkinter.Label(self.frame, textvariable=self.time).grid(
            row=0, column=0, columnspan=2, sticky=(tkinter.N, tkinter.W))
        tkinter.Label(self.frame, textvariable=self.battery).grid(
            row=0, column=2, columnspan=2, sticky=(tkinter.N, tkinter.E))
        tkinter.Label(self.frame, textvariable=self.strength).grid(
            row=1, column=1, columnspan=2, sticky=(tkinter.N, tkinter.E))
        tkinter.Label(self.frame, textvariable=self.channel).grid(
            row=1, column=0, columnspan=2, sticky=(tkinter.N, tkinter.W))
        self.__clock()

        # set up buttons
        # they do nothing for now.
        tkinter.Button(self.frame, text="1", command=self.button1).grid(
            row=3, column=0, sticky=(tkinter.N, tkinter.W, tkinter.E, tkinter.S))
        tkinter.Button(self.frame, text="2").grid(
            row=3, column=1, sticky=(tkinter.N, tkinter.W, tkinter.E, tkinter.S))
        tkinter.Button(self.frame, text="3").grid(
            row=3, column=2, sticky=(tkinter.N, tkinter.W, tkinter.E, tkinter.S))
